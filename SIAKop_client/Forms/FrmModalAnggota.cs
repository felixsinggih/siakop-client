...
namespace SIAKop_client.Forms {
    public partial class FrmModalAnggota : Form {
        ...
        public FrmModalAnggota(FrmKredit kre) {
            InitializeComponent();
            this.kredit = kre;
        }

        private void FrmModalAnggota_Load(object sender, EventArgs e) {
        ...
        private void DgvAnggota_CellContentClick(object sender, DataGridViewCellEventArgs e) {
            if (DgvAnggota.Columns[e.ColumnIndex].Name == "set") {
                DataGridViewRow row = this.DgvAnggota.Rows[e.RowIndex];
                kredit.idAnggota = row.Cells["din"].Value.ToString();
                kredit.actAng = true;
                using (FrmWait frm=new FrmWait(kredit.SearchAnggota)) {
                    frm.ShowDialog();
                }
                this.Close();
            }
